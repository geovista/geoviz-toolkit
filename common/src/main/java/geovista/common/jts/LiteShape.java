 ...
 * @author Andrea Aime
 * @source $URL:
 *         http://svn.geotools.org/geotools/trunk/gt/modules/library/main/src
 *         /main/java/org/geotools/geometry/jts/LiteShape.java $
 * 
 */
public class LiteShape implements Shape, Cloneable {
	...
	private final GeomCollectionIterator collIterator = new GeomCollectionIterator();
	 ...
	 * @param at
	 *            - the transformation applied to the geometry in order to get
	 *            to the shape points
	 * @param generalize
	...
	public LiteShape(Geometry geom, AffineTransform at, boolean generalize) {
		if (geom != null) {
			geometry = getGeometryFactory().createGeometry(geom);
		}
		affineTransform = at;
	 ...
	 * 
	 * <ul>
	 * <li>the <code>intersect</code> method returns <code>true</code> and</li>
	...
	public Rectangle2D getBounds2D() {
		...
		return new Rectangle2D.Double(env.getMinX(), env.getMinY(), env
	...
	public PathIterator getPathIterator(AffineTransform at) {
		...
		if (geometry instanceof Polygon) {
			...
			// pi = new PackedLineIterator((LineString) geometry, combined,
			// generalize,
			// (float) maxDistance);
			// else
			...
			collIterator.init((GeometryCollection) geometry, combined,
					generalize, maxDistance);
			pi = collIterator;
		}
