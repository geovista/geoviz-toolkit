...
public class StereographicAzimuthalProjection extends AzimuthalProjection {
	...
	public void setupUPS(int pole) {
		projectionLatitude = (pole == SOUTH_POLE) ? -MapMath.HALFPI
				: MapMath.HALFPI;
	...
	public void initialize() {
		...
		if (spherical) {
			...
			case SOUTH_POLE:
				if (Math.abs(trueScaleLatitude - MapMath.HALFPI) < EPS10) {
					...
					akm1 = Math.cos(trueScaleLatitude)
							/ MapMath.tsfn(trueScaleLatitude, t = Math
									.sin(trueScaleLatitude), e);
					t *= e;
			...
			case OBLIQUE:
				...
				X = 2. * Math.atan(ssfn(projectionLatitude, t, e))
						- MapMath.HALFPI;
				t *= e;
				akm1 = 2. * scaleFactor * Math.cos(projectionLatitude)
						/ Math.sqrt(1. - t * t);
				sinphi0 = Math.sin(X);
				cosphi0 = Math.cos(X);
				break;
				...
				akm1 = Math.abs(trueScaleLatitude - MapMath.HALFPI) >= EPS10
						? Math.cos(trueScaleLatitude)
								/ Math.tan(MapMath.QUARTERPI - .5
										* trueScaleLatitude) : 2. * scaleFactor;
				break;
			}
		}
	}
	...
	public Point2D.Double project(double lam, double phi, Point2D.Double xy) {
		...
		if (spherical) {
			...
			case SOUTH_POLE:
				...
				xy.x = sinlam
						* (xy.y = akm1 * Math.tan(MapMath.QUARTERPI + .5 * phi));
				xy.y *= coslam;
				break;
			}
		} else {
			...
			case OBLIQUE:
				A = akm1
						/ (cosphi0 * (1. + sinphi0 * sinX + cosphi0 * cosX
								* coslam));
	...
	public Point2D.Double projectInverse(double x, double y, Point2D.Double lp) {
		if (spherical) {
			...
			sinc = Math.sin(c = 2. * Math.atan((rh = MapMath.distance(x, y))
					/ akm1));
			cosc = Math.cos(c);
			lp.x = 0.;
			switch (mode) {
