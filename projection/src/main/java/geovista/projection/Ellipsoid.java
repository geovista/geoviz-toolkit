...
public class Ellipsoid implements Cloneable {
	...
	public final static Ellipsoid SPHERE = new Ellipsoid("sphere",
			6371008.7714, 6371008.7714, 0.0, "Sphere");
	...
	public final static Ellipsoid WGS_1960 = new Ellipsoid("WGS60", 6378165.0,
			0.0, 298.3, "WGS 60");
	public final static Ellipsoid WGS_1966 = new Ellipsoid("WGS66", 6378145.0,
			0.0, 298.25, "WGS 66");
	public final static Ellipsoid WGS_1972 = new Ellipsoid("WGS72", 6378135.0,
			0.0, 298.26, "WGS 72");
	public final static Ellipsoid WGS_1984 = new Ellipsoid("WGS84", 6378137.0,
			0.0, 298.257223563, "WGS 84");
	public final static Ellipsoid KRASOVSKY = new Ellipsoid("krass", 6378245.0,
			298.3, 0.0, "Krassovsky, 1942");
	public final static Ellipsoid EVEREST = new Ellipsoid("evrst30",
			6377276.345, 0.0, 300.8017, "Everest 1830");
	public final static Ellipsoid INTERNATIONAL_1967 = new Ellipsoid(
			"new_intl", 6378157.5, 6356772.2, 0.0, "New International 1967");
	public final static Ellipsoid GRS_1980 = new Ellipsoid("GRS80", 6378137.0,
			0.0, 298.257222101, "GRS 1980 (IUGG, 1980)");
	...
	public final static Ellipsoid[] ellipsoids = {
			...
			new Ellipsoid("SGS85", 6378136.0, 0.0, 298.257,
					"Soviet Geodetic System 85"),
			GRS_1980,
			new Ellipsoid("IAU76", 6378140.0, 0.0, 298.257, "IAU 1976"),
			AIRY,
			new Ellipsoid("APL4.9", 6378137.0, 0.0, 298.25,
					"Appl. Physics. 1965"),
			new Ellipsoid("NWL9D", 6378145.0, 298.25, 0.0,
					"Naval Weapons Lab., 1965"),
			new Ellipsoid("mod_airy", 6377340.189, 6356034.446, 0.0,
					"Modified Airy"),
			new Ellipsoid("andrae", 6377104.43, 300.0, 0.0,
					"Andrae 1876 (Den., Iclnd.)"),
			new Ellipsoid("aust_SA", 6378160.0, 0.0, 298.25,
					"Australian Natl & S. Amer. 1969"),
			new Ellipsoid("GRS67", 6378160.0, 0.0, 298.2471674270,
					"GRS 67 (IUGG 1967)"),
			BESSEL,
			new Ellipsoid("bess_nam", 6377483.865, 0.0, 299.1528128,
					"Bessel 1841 (Namibia)"),
			CLARKE_1866,
			CLARKE_1880,
			new Ellipsoid("CPM", 6375738.7, 0.0, 334.29,
					"Comm. des Poids et Mesures 1799"),
			new Ellipsoid("delmbr", 6376428.0, 0.0, 311.5,
			...
			new Ellipsoid("evrstSS", 6377298.556, 0.0, 300.8017,
					"Everest (Sabah & Sarawak)"),
			new Ellipsoid("fschr60", 6378166.0, 0.0, 298.3,
					"Fischer (Mercury Datum) 1960"),
			new Ellipsoid("fschr60m", 6378155.0, 0.0, 298.3,
			...
			new Ellipsoid("intl", 6378388.0, 0.0, 297.0,
			...
			INTERNATIONAL_1967,
			new Ellipsoid("plessis", 6376523.0, 6355863.0, 0.0,
					"Plessis 1817 France)"),
			new Ellipsoid("SEasia", 6378155.0, 6356773.3205, 0.0,
					"Southeast Asia"),
			new Ellipsoid("walbeck", 6376896.0, 6355834.8467, 0.0, "Walbeck"),
			WGS_1960, WGS_1966, WGS_1972, WGS_1984, };

	public Ellipsoid() {
	}

	// One of of poleRadius or reciprocalFlattening must be specified, the other
	// zero
