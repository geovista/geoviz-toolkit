...
public class MunsellChooserPanel extends AbstractColorChooserPanel {
	...
	public void buildChooser() {
		initValues();
		initColors();
		holderPanel.setLayout(new GridLayout(numSwatches.height,
				numSwatches.width, gap.width, gap.height));
		for (MunsellSwatch swatche : swatches) {
			holderPanel.add(swatche);
		}
	...
	protected void initColors() {
		...
		swatches = new MunsellSwatch[numColors];
		for (int i = 0; i < numColors; i++) {
			colors[i] = new Color(rawValues[(i * 3)], rawValues[(i * 3) + 1],
					rawValues[(i * 3) + 2]);
			swatches[i] = new MunsellSwatch();
			...
			swatches[i].setBorder(BorderFactory
					.createBevelBorder(BevelBorder.RAISED));
		}

	}

	private class MunsellSwatch extends JPanel implements MouseListener {
		transient private String tipText;
		...
		public void mousePressed(MouseEvent e) {
			getColorSelectionModel().setSelectedColor(getBackground());
		}
		...
		public void mouseEntered(MouseEvent e) {
			...
			tipText = "Red = " + c.getRed() + " Green = " + c.getGreen()
					+ " Blue = " + c.getBlue();
	...
	public static void main(String[] args) {
		...
		MunsellChooserPanel swatPanel = new MunsellChooserPanel();
